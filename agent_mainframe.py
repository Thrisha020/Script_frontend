...
def run_mainframe_commands(ssh, hlq, application, filename, file_path, config, workspace, outdir):
    try:
        ...
        uss_command = (
            f"{groovyzpath} -DBB_PERSONAL_DAEMON "
            f"{zappbuildpath} --workspace {workspace} "
...
def handle_server_command(action, config_path):
    ...
    if action == "git_automation":
        if not os.path.isdir(clone_path):
            check_internet_connection()
        else:
            print(f"Repository already exists at {clone_path}.")
    """elif action == "pull_changes":
        if os.path.isdir(clone_path):
            pull_latest_changes(clone_path)
        else:
            print(f"Repository not found at {clone_path}. Please clone it first.")
        ...
        if os.path.isfile(file_path):
            ...
            print(f"File {file_name} does not exist at {clone_path}.")
    else:
